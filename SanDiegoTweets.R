...
#creates vector to hold the number of tweets in San Diego per hour
...
for(i in 1:length(files)){
	...
	if(!is.null(sick)){
		...
		if(length(a)>0){
			#assigns the number of tweets in the file to the ith value of the time vector
			time[i] <- length(a)
		}
	}
	#prints file number
...
#plots the number of tweets in San Diego per hour
